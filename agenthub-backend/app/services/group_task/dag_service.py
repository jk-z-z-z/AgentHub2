...
from app.services.group_task.helpers import ensure_group_member, validate_dag_nodes
...
def _resolve_node_key_map(rows: list[GroupTaskNode]) -> dict[str, GroupTaskNode]:
    return {row.node_key: row for row in rows}


def list_nodes(db: Session, *, group_id: int) -> list[GroupTaskNode]:
    return (
        db.query(GroupTaskNode)
        .filter(GroupTaskNode.group_id == int(group_id))
...
def get_node_by_key(db: Session, *, group_id: int, node_key: str) -> GroupTaskNode | None:
    return (
        db.query(GroupTaskNode)
        .filter(GroupTaskNode.group_id == int(group_id), GroupTaskNode.node_key == str(node_key))
        .first()
    )
...
def _row_snapshot(row: GroupTaskNode, rows_by_id: dict[int, GroupTaskNode]) -> dict:
    ...
    return {
        ...
        "error": row.error,
        "agent_run_id": int(row.agent_run_id) if row.agent_run_id else None,
...
def get_dag_view(db: Session, *, group_id: int) -> dict:
    ...
    nodes_by_key = _resolve_node_key_map(rows)
    edges: list[dict] = []
    for row in rows:
...
def get_active_node(db: Session, *, group_id: int) -> GroupTaskNode | None:
    rows = list_nodes(db, group_id=int(group_id))
    for node in rows:
...
def resolve_node(
    ...
    *,
    ...
    if node_id is not None:
        ...
        return node
    if group_id is None:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="node_id or group_id is required")
    node = get_active_node(db, group_id=int(group_id))
    if not node:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Active node not found")
    return node
...
def _snapshot_from_db(db: Session, *, group_id: int) -> list[dict]:
    rows = list_nodes(db, group_id=int(group_id))
    rows_by_id = {int(row.id): row for row in rows}
...
def patch_dag(
    ...
    *,
    ...
    current_rows = list_nodes(db, group_id=int(group_id))
    ...
    try:
        for raw_op in ops:
            ...
            if op == "add_node":
                payload = _normalize_node_payload(raw_op.get("node") or {})
                ...
                payload_nodes = raw_op.get("nodes")
                ...
                node_keys = raw_op.get("node_keys")
        ...
        snapshot = _snapshot_from_db(db, group_id=int(group_id))
...
def patch_nodes(
    ...
    *,
    ...
    return create_nodes(db, group_id=int(group_id), creator_member_id=int(creator_member_id), nodes=nodes)
