...
class ClaudeCodeEngine(BaseAgentEngine):
    async def run(
        self,
        *,
        ctx: EngineContext,
        ...
        if tool_executor is None:
            raise ValueError("Claude Code engine requires tool_executor")

        claude_command = str(getattr(settings, "acp_claude_command", "") or "python -m claude_code_acp")
