...
class FileReadTool(ToolBase):
    ...
    def __init__(self) -> None:
        self.name = "bootstrap.file_read"
        self.description = "Read files under the agent bootstrap workspace."
        self.input_schema = {
            "type": "object",
    ...
    async def __call__(self, **kwargs) -> ToolChunk:
        ...
        if not target.exists() or not target.is_file():
            return build_error_chunk("file_not_found")
        return build_tool_chunk({"path": target.relative_to(agent_root(int(agent_id))).as_posix(), "content": target.read_text(encoding="utf-8")})
