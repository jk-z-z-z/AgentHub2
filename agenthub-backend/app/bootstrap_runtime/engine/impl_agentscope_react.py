...
from app.bootstrap_runtime.engine.base import BaseBootstrapEngine, BootstrapEngineContext
...
class BootstrapAgentScopeEngine(BaseBootstrapEngine):
    async def run(self, *, ctx: BootstrapEngineContext, req: Any) -> tuple[str, dict[str, Any]]:
        ...
        agent = Agent(
            name=f"bootstrap-agent-{ctx.agent_id}",
