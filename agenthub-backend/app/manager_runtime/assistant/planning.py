...
def manager_tool_upsert_plan(
    ...
    *,
    ...
    if str(active.status or "") in {"completed", "cancelled"}:
        created = manager_tool_write_plan_run(
            db,
            group_id=int(group_id),
            creator_member_id=int(creator_member_id),
    ...
    update_group_task_dag(db, run_id=int(active.id), nodes=list(plan.get("nodes") or []))
    active.title = str(plan.get("plan_title") or active.title)
    active.goal_text = str(plan.get("goal") or active.goal_text)
